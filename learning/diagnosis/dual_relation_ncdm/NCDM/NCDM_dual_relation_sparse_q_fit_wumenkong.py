...
class NoGatePropagationQNet(nn.Module):
    """
    No-gate relation-propagation Q-net.
    It directly combines original Q, prerequisite residuals, and similarity residuals.
    """
    def __init__(
        self,
        knowledge_n: int,
    ...
    def forward(
        ...
        input_knowledge_point = q_original + q_prereq_residual + q_similarity_residual
...
class NCDM(CDM):
    """
    NCDM wrapper for the no-gate relation-propagation variant.
    """
    def __init__(
        self,
        knowledge_n: int,
        ...
        super().__init__()
        self.ncdm_net = NoGatePropagationQNet(knowledge_n, exer_n, student_n)
        self.prereq_gate_l1_weight = 0.0
        self.similarity_gate_l1_weight = 0.0
    ...
    def predict_scores(
        ...
        data_loader,
        device: str = "cpu",
        force_zero_gate: bool = False,   # 蹇界暐
        ...
        with torch.no_grad():
            for user_id, item_id, q_original, q_prereq_residual, q_similarity_residual, y in tqdm(data_loader, "Evaluating"):
                ...
                pred = self.ncdm_net(user_id, item_id, q_original, q_prereq_residual, q_similarity_residual)
                y_pred.extend(pred.detach().cpu().tolist())
                y_true.extend(y.tolist())

        gate_stats = {
            "prereq_gate_mean": 0.0,
            "prereq_gate_max": 0.0,
            "similarity_gate_mean": 0.0,
            "similarity_gate_max": 0.0,
        }
    ...
    def _collect_full_q_matrices(self, data_loader, device: str = "cpu") -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """Collect full Q matrices for final propagated-Q export."""
    ...
    def export_q_matrix(
        ...
        """Save the no-gate propagated Q matrix."""
        self.ncdm_net.eval()
        ...
        with torch.no_grad():
            q_adjusted = q_original + q_prereq_residual + q_similarity_residual

        df = pd.DataFrame(q_adjusted.cpu().numpy())
        df.to_csv(output_path, index=False, header=False)
        print(f"Propagated Q matrix (no gate) saved to {output_path}")
    ...
    def train(
        ...
        if q_save_path is not None and q_orig_full is not None:
            print("Saving propagated Q matrix after training...")
