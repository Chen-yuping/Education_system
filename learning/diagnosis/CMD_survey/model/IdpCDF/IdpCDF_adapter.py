...
# 将 BaseModel 模型目录加入 sys.path（以便导入其内部模块）
basecdm_dir = os.path.join(settings.BASE_DIR, 'learning', 'CMD_survey', 'model', 'IdpCDF')
if basecdm_dir not in sys.path:
    sys.path.insert(0, basecdm_dir)

# 导入 ConCDF 模型及相关函数
from IdpCDF import IRTCDM
...
def train_basecdm(dataset_name, device='cpu'):
    ...
    hparams['base_log_path'] = log_path

    # 实例化模型
    model = IRTCDM(
        n_user=n_user, n_item=n_item, n_know=n_know,
        hidden_dim=hidden_dim,
        itf_type='irt', log_path=log_path
    )
    model.to(device)
