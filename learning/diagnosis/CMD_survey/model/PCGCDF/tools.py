...
def labelize(y_pred: torch.DoubleTensor, threshold = 0.5)->np.ndarray:
    return (y_pred > threshold).to('cpu').detach().numpy().astype(np.int_).reshape(-1,)
...
def format_hparams(hparams: dict) -> str:
    if not hparams:
        return 'hparams = {}'
    lines = ['hparams = {']
    for idx, (key, value) in enumerate(hparams.items()):
        suffix = ',' if idx < len(hparams) - 1 else ''
        lines.append('  {}: {}{}'.format(key, value, suffix))
    lines.append('}')
    return '\n'.join(lines)
